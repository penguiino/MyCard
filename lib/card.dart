...
class _CardPageState extends State<CardPage> {
  ...
  Future<void> _fetchUserDetails() async {
    try {
      ...
      if (user != null) {
        ...
        final data = userDoc.data() as Map<String, dynamic>?; // Cast the data to a map
        if (data != null) {
  ...
  void _shareQRCode() {
    try {
      User? user = FirebaseAuth.instance.currentUser;
      if (user != null) {
        String qrData = user.uid; // Use the UID of the current user as QR code data
                      ...
                      Row(
                        ...
                        children: [
                          ...
                          Column(
                            ...
                            children: [
                              Text(
                                _userName ?? 'John Doe', // Display the user's title if available, otherwise use a default value
                        ...
                        children: [
                          Row(
                            children: [
                              ...
                              Text(
                                _userTitle ?? 'Title placeholder', // Display the user's title if available, otherwise use a default value
                          ...
                          Row(
                            children: [
                              ...
                              Text(
                                _userNumber ?? 'number placeholder', // Display the user's title if available, otherwise use a default value
                                style: const TextStyle(
                                  fontSize: 18,
                                  fontWeight: FontWeight.bold,
                                  color: Colors.black,
                          ...
                          Row(
                            children: [
                              ...
                              Text(
                                _userEmail ?? 'email placeholder', // Display the user's title if available, otherwise use a default value
                                style: const TextStyle(
                                  fontSize: 18,
                                  fontWeight: FontWeight.bold,
                                  color: Colors.black,
...
class _QRScanPageState extends State<QRScanPage> {
  ...
  void _checkContactExistence(String? contactUID) async {
    ...
    try {
      User? user = FirebaseAuth.instance.currentUser;
      if (user != null) {
        // Check if contactUID exists in the user's contacts list
        DocumentSnapshot userDoc = await FirebaseFirestore.instance.collection('users').doc(user.uid).get();
        final data = userDoc.data() as Map<String, dynamic>?; // Cast the data to a map
        List<dynamic> contacts = data?['contacts'] ?? [];
        if (!contacts.contains(contactUID)) {
          // If contactUID doesn't exist, add it to the contacts list
