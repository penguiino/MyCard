...
class _SignUpDemoState extends State<SignUpDemo> {
  ...
  Future signUp() async {
    if (passwordConfirmed()) {
      try {
        ...
        if (user != null) {
          // If user is created successfully, add user details to Firestore
          String uid = user.uid;
          addUserDetails(
            uid,
  ...
  Future<void> addUserDetails(
      ...
      List<String> contacts, // Add contacts parameter
      ) async {
    try {
      await FirebaseFirestore.instance.collection('users').doc(uid).set({
        ...
        'contacts': contacts, // Initialize contacts as an empty array
      });
